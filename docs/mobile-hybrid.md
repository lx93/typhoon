...
Typhoon should use a hybrid mobile architecture: shared cross-platform UI later, native VPN transport always.
...
The iOS scaffold in `ios/` already follows this split. The SwiftUI host app is temporary MVP UI, and the packet tunnel now owns the sing-box/libbox engine behind a small Swift boundary that can later sit behind a React Native native module.
