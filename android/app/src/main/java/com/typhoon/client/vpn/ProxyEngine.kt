...
class LibboxProxyEngine : ProxyEngine {
    ...
    override suspend fun start(
        ...
        val platform = TyphoonLibboxPlatform(vpnService) { fd ->
            tunFd?.close()
            tunFd = fd
        }
        val handler = TyphoonCommandServerHandler(::stop)
        val options = SetupOptions().apply {
            ...
            logMaxLines = 3000
            debug = true
            crashReportSource = "TyphoonAndroid"
...
private class TyphoonCommandServerHandler(
    ...
    override fun writeDebugMessage(message: String?) {
        message
            ...
            ?.forEach {
                Log.d(LOG_TAG, it)
                TyphoonStatusStore.appendLog("libbox: $it")
            }
    }
}

private class TyphoonLibboxPlatform(
    ...
    override fun findConnectionOwner(
        ...
        throw UnsupportedOperationException("connection owner lookup is not supported")
    }
...
private const val LOG_TAG = "TyphoonLibbox"
